...
Usage: (most of the options have valid default values this is an extended example for demonstration purposes)
    ...
    --weight_decay 0.005 --lr_scheduler_type "linear" --seed 3407 --out_dir "outs" \
...
def run(args):
    ...
    def formatting_prompts_func(examples):
        ...
        outs      = examples["out"]
        texts = []
        for instruction, input, out in zip(instructions, inputs, outs):
            text = alpaca_prompt.format(instruction, input, out) + EOS_TOKEN
            texts.append(text)
        return {"text": texts}
...
if __name__ == "__main__":
    ...
    training_group.add_argument('--per_device_train_batch_size', type=int, default=2, help="Batch size per during training, default is 2.")
    ...
    save_group.add_argument('--out_dir', type=str, default="outs", help="Out directory")
