...
def LlamaAttention_fast_forward_inference(
    ...
    if do_prefill:
        self.paged_attention = torch.empty((KV_CACHE_INCREMENT+seq_len+1, 2, bsz, n_kv_heads, head_dim), dtype = dtype, device = ":0")
        self.paged_attention_K = self.paged_attention[:,0]
        self.paged_attention_V = self.paged_attention[:,1]
        self.paged_attention_K[:seq_len] = K1.permute(2, 0, 1, 3)
        self.paged_attention_V[:seq_len] = V1.permute(2, 0, 1, 3)
        self.temp_QA = torch.empty((2, bsz, 1, attention_size), dtype = dtype, device = ":0")
        self.temp_KV = torch.empty((2, bsz, 1, n_kv_heads*head_dim), dtype = dtype, device = ":0")
        self.RH_Q = torch.empty((bsz, n_heads, 1, head_dim), dtype = dtype, device = ":0")
        
        # Mistral Nemo 12b has weird dimensions
        if attention_size != hidden_size:
            self.temp_O = torch.empty((1, bsz, hidden_size), dtype = dtype, device = ":0")
        else:
            self.temp_O = self.temp_QA[1][:,:,:hidden_size]
        pass
        
        self.attention = torch.empty((bsz, n_heads, 1, KV_CACHE_INCREMENT+seq_len), dtype = dtype, device = ":0")
    ...
    RH_K = RH_Q[:,:n_kv_heads,:,:] # torch.empty((n_kv_heads, 1, head_dim), dtype = dtype, device = ":0")
    RH_K[:,:,:,:h] = Kn[:,:,:,h:]
    RH_K[:,:,:,h:] = Kn[:,:,:,:h]
...
def fast_swiglu_inference(self, X, temp_gate = None, temp_up = None):
    ...
    # temp = torch.empty((2, bsz, 1, mlp_size), dtype = X.dtype, device = ":0")
...
def LlamaAttention_fast_forward(
    ...
    *args, **kwargs,
    ...
    if (not HAS_FLASH_ATTENTION and HAS_XFORMERS and attention_mask is None):
        ...
        pass
    pass
    attn_out = A.reshape(bsz, q_len, n_heads*head_dim)
    attn_out = self.apply_o(self, attn_out)
    attn_weights = None
...
def LlamaDecoderLayer_fast_forward(
    ...
    position_ids:        Optional[torch.LongTensor] = None,
    past_key_value:      Optional[Tuple[torch.Tensor]] = None,
    out_attentions:   Optional[bool] = False,
    ...
    *args, **kwargs,
    ...
    outs = (hidden_states,)
    if out_attentions: outs += (self_attn_weights,)
    if use_cache: outs += (present_key_value,)
    return outs
pass
...
def LlamaModel_fast_forward(
    ...
    out_attentions:    Optional[bool] = None,
    out_hidden_states: Optional[bool] = None,
    return_dict:          Optional[bool] = None,
    *args, **kwargs,
) -> Union[Tuple, BaseModelOutWithPast]:
    
    out_attentions = out_attentions if out_attentions is not None else self.config.out_attentions
    assert(out_attentions is False)
    out_hidden_states = (
        out_hidden_states if out_hidden_states is not None else self.config.out_hidden_states
    )
    ...
    # decoder layers
    all_hidden_states = () if out_hidden_states else None
    all_self_attns = () if out_attentions else None
    ...
    if (self.gradient_checkpointing and self.training and not use_cache):
        ...
        if out_attentions is False and hasattr(self, "_offloaded_gradient_checkpointing"):
        ...
        if HAS_FLASH_ATTENTION_SOFTCAPPING and attention_mask is None:
            ...
            if HAS_FLEX_ATTENTION:
                ...
                self.SWA_mask = AttentionMaskConverter(
                    ...
                    .to_causal_4d(1, n, n, dtype = inputs_embeds.dtype, device = ":0",)\
                    .squeeze(0).squeeze(0)
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        if out_hidden_states: all_hidden_states += (hidden_states,)
        ...
        if offloaded_gradient_checkpointing:
            ...
            def create_custom_forward(module):
                def custom_forward(*inputs):
                    return module(*inputs, past_key_value, out_attentions, padding_mask = padding_mask, position_embeddings = position_embeddings)
        ...
        if use_cache: next_decoder_cache += (layer_outs[2 if out_attentions else 1],)
        if out_attentions: all_self_attns += (layer_outs[1],)
    pass
...
def LlamaModel_fast_forward_inference(
    ...
    residual = torch.empty((bsz, q_len, hd), dtype = torch.float32, device = ":0")
    _XX = torch.empty((2, bsz, q_len, hd), dtype = torch.float32, device = ":0")
    XX, XX2 = _XX[0], _XX[1]
    variance = torch.empty((bsz, q_len, 1), dtype = torch.float32, device = ":0")
    temp_mlp = torch.empty((2, bsz, 1, mlp_size), dtype = X.dtype, device = ":0")
...
def CausalLM_fast_forward(fast_forward_inference):
    def _CausalLM_fast_forward(
        ...
        *args, **kwargs,
    ) -> Union[Tuple, CausalLMOutWithPast]:
        
        if past_key_values is not None:
            outs = fast_forward_inference(
                self,
                input_ids,
                past_key_values,
                position_ids = position_ids,
            ...
            # decoder outs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        ...
        # Out last hidden states without logits if asked
        if os.environ.get("UNSLOTH_RETURN_HIDDEN_STATES", "0") == "1":
            if num_logits_to_keep != 0:
                hidden_states = hidden_states[:, -num_logits_to_keep:, :]
            return CausalLMOutWithPast(
                loss = None,
        ...
        if labels is not None:
            ...
            if not hasattr(self, "extra_ignored_labels"):
                ...
                self.extra_ignored_labels = torch.full((self.max_seq_length, 1), -100, device = ":0")
            pass
...
# Inferene can now be Graphed, but we shall retain the old rotary embeddings.
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, dim = None, max_position_embeddings=2048, base=10000, device=None,
        ...
        self._set_cos_sin_cache(seq_len=self.current_rope_size, device, dtype=torch.get_default_dtype())
    pass

    def _set_cos_sin_cache(self, seq_len, device, dtype):
        # Note: on the original Llama codebase, these tensors are created on the target (and not on ) and
        # in FP32. They are applied (multiplied) in FP32 as well.
        self.current_rope_size = seq_len
        inv_freq = 1.0 / (
            self.base ** (torch.arange(0, self.dim, 2, dtype=torch.int64, device="").float() / self.dim)
        )
        t = torch.arange(self.current_rope_size, device="", dtype=torch.int64).float()

        freqs = torch.outer(t, inv_freq)
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = torch.cat((freqs, freqs), dim=-1)
        self.register_buffer("cos_cached", emb.cos().to(dtype=dtype, device, non_blocking=True), persistent=False)
        self.register_buffer("sin_cached", emb.sin().to(dtype=dtype, device, non_blocking=True), persistent=False)
    pass

    def forward(self, x, position_ids=None, seq_len=None):
        # x: [bs, num_attention_heads, seq_len, head_size]
        if seq_len > self.current_rope_size:
            self._set_cos_sin_cache(seq_len=seq_len, device=x, dtype=x.dtype)
    ...
    def extend_rope_embedding(self, x, seq_len):
        ...
        self._set_cos_sin_cache(self.current_rope_size, device = ":0", dtype = x.dtype)
    pass
pass
...
class LlamaExtendedRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = torch.arange(self.current_rope_size, device=self.inv_freq, dtype=torch.int64).float()
    ...
    def apply_scaling(self, freqs: torch.Tensor):
        ...
        return torch.tensor(new_freqs, dtype=freqs.dtype, device=freqs)
    pass
...
class LongRopeRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self,
        ...
        inv_freq_shape = torch.arange(0, self.dim, 2, dtype=torch.int64, device="").float() / self.dim
        short_factor = torch.tensor(short_factor, device = "", dtype = torch.float32)
        long_factor  = torch.tensor(long_factor,  device = "", dtype = torch.float32)
        ...
        t = torch.arange(original_max_position_embeddings, device=self.short_inv_freq, dtype=torch.int64).float()
        freqs = torch.outer(t, self.short_inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1)
        cos_cached = (emb.cos() * self.scaling_factor).to(dtype=dtype, device, non_blocking=True)
        sin_cached = (emb.sin() * self.scaling_factor).to(dtype=dtype, device, non_blocking=True)
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = torch.arange(self.current_rope_size, device=self.long_inv_freq, dtype=torch.int64).float()
        # Long sequences
...
class FastLlamaModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        _stats = torch..get_device_properties(0)
        max_memory = round(_stats.total_memory / 1024 / 1024 / 1024, 3)

        statistics = \
           f"==((====))==  Unsloth {__version__}: Fast {model_patcher.__name__[4:-5]} patching. Transformers: {transformers_version}.\n"\
           f"   {chr(92)}{chr(92)}   /|    : {_stats.name}. Max memory: {max_memory} GB. Platform: {platform_system}.\n"\
           f"O^O/ {chr(92)}_/ {chr(92)}    Torch: {torch.__version__}. : {_stats.major}.{_stats.minor}.  Toolkit: {torch.version.}. Triton: {triton_version}\n"\
        ...
        if not fast_inference:
            ...
            load_vllm_kwargs = dict(
                ...
                config                 = model_config,
                _memory_utilization = _memory_utilization,
        ...
        try:
            ...
            raise RuntimeError('Unsloth currently does not support multi  setups - but we are working on it!')
        pass
        ...
        f"==((====))==  Unsloth - 2x faster free finetuning | Num s = {args.world_size}\\n"\\
        f"   {chr(92)}{chr(92)}   /|    Num examples = {num_examples:,} | Num Epochs = {num_train_epochs:,}\\n"\\
        f"O^O/ {chr(92)}_/ {chr(92)}    Batch size per = {self._train_batch_size:,} | Gradient Accumulation steps = {args.gradient_accumulation_steps}\\n"\\
        ...
        a = np.array([0,])
        try:
            a = subprocess.check_out('nvidia-smi --query-=memory.used --format=csv', shell = True)
        ...
        if n_total_devices > 1:
            logger.warning_once('Unsloth currently does not support multi  setups - but we are working on it!')
        debug_info ="""
        debug_info = debug_info.split('\n')
        debug_info = "\n".join([debug_info[0]] + [spaces + x[8:] for x in debug_info[1:]])
        ...
        inner_training_loop = inner_training_loop.replace(
            "train_dataloader = _spmd_dataloader(train_dataloader)",
            "raise RuntimeError('Unsloth: s are not yet supported!')"
        )
        inner_training_loop = inner_training_loop.replace(
        ...
        **kwargs,
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            if check_all:
                ...
                # Offload!
                # [TODO] First offload lm_head and embed_tokens to  (should be disk!!)
                if "embed_tokens" in new_target_modules:
                    ...
                    model.get_input_embeddings().modules_to_save.default\
                        .to = ":0", dtype = new_dtype, non_blocking = True)
                    model.get_input_embeddings().modules_to_save.default.requires_grad_(True)

                    # [TODO] Move old embed_tokens to  - should be disk!
                ...
                if "lm_head" in new_target_modules:
                    ...
                    new_dtype = model.get_out_embeddings().modules_to_save.default.weight.dtype
                    if new_dtype == torch.float16:
                        # See https://github.com/unslothai/unsloth/pull/1200
                        # Tesla T4 must use float32 and not float16
                        new_dtype = torch.float32
                    ...
                    model.get_out_embeddings().modules_to_save.default\
                        .to = ":0", dtype = new_dtype, non_blocking = True)
                    model.get_out_embeddings().modules_to_save.default.requires_grad_(True)

                    # [TODO] Move old lm_head to  - should be disk!
                    model.get_out_embeddings().original_module\
                        .to = "", non_blocking = True)
                    model.get_out_embeddings().original_module.requires_grad_(False)
                pass
        ...
        input_embeddings_device  = model. get_input_embeddings().weight
        out_embeddings_device = model.get_out_embeddings().weight
        ...
        if use_gradient_checkpointing == "unsloth":
            ...
            if train_lm_head:
                print("Unsloth: Offloading out_embeddings to disk to save VRAM")
                offload_out_embeddings(model, temporary_location)
            pass
        ...
        if train_lm_head:
            print("Unsloth: Training lm_head in mixed precision to save VRAM")
            assert(hasattr(model.get_out_embeddings(), "modules_to_save"))
    ...
    def patch_peft_model(
        ...
        extra_ignored_labels = torch.full((max_seq_length, 1), -100, device = ":0")
    ...
    def for_inference(model):
        ...
        pass
        if hasattr(model, "get_out_embeddings"):
            embeddings = model.get_out_embeddings()
            if hasattr(embeddings, "training"): embeddings.training = False
        pass
