...
# The speedups for torchdynamo mostly come wih  Ampere or higher and which is not detected here.
...
# =============================================
# torch..amp.custom_fwd is deprecated >= 2.4
torch_version = torch.__version__
if Version(torch_version) < Version("2.4.0"):
    torch_amp_custom_fwd = torch..amp.custom_fwd
    torch_amp_custom_bwd = torch..amp.custom_bwd
else:
    torch_amp_custom_fwd = torch.amp.custom_fwd(device_type = "")
    torch_amp_custom_bwd = torch.amp.custom_bwd(device_type = "")
pass
# =============================================
...
major_version, minor_version = torch..get_device_capability()
...
if major_version >= 8:
    ...
    if _is_package_available("flash_attn"):
        # Check for  linking errors "undefined symbol: _ZNK3c106SymIntltEl"
        try:
            try:
                # See https://github.com/unslothai/unsloth/issues/1437
                from flash_attn.flash_attn_interface import flash_attn_
            except:
                from flash_attn.flash_attn_interface import flash_attn_
            ...
            print(
                ...
                "A possible explanation is you have a new  version which isn't\n"\
...
def is_big_(index):
    sms = torch..get_device_properties(index).multi_processor_count
    if sms < 80:  # V100
        # log.warning("not enough SMs to use max_autotune_gemm mode")
        return False
    return True
import torch._inductor.utils
torch._inductor.utils.is_big_ = is_big_
patch_torch_compile(
...
def prepare_model_for_kbit_training(
    ...
    if use_reentrant:
        if hasattr(model, "enable_input_require_grads"):
            ...
            def make_inputs_require_grad(module, input, out):
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        end = source.find("self.to(weight)", start)
...
def _get_statistics(statistics = None, force_download = True):
    ...
    try:
        ...
        elif "\nCOLAB_"  in keynames and n_s == 1: statistics = "colab"
...
def get_statistics():
    ...
    try:
        vram = torch..get_device_properties(0).total_memory / 1024 / 1024 / 1024
        if   vram <= 8 : vram = 8
...
def _prepare_backend(
    self,  = False, sagemaker_dp = False, backend: str = None,
...
def offload_to_disk(W, model, name, temporary_location : str = "_unsloth_temporary_saved_buffers"):
    ...
    offloaded_W = torch.load(filename, map_location = "", mmap = True)
...
def offload_out_embeddings(model, temporary_location : str = "_unsloth_temporary_saved_buffers"):
    offloaded_W = offload_to_disk(model.get_out_embeddings(), model, "out_embeddings", temporary_location)

    new_out_embeddings = torch.nn.Linear(1, 1, bias = None)
    del new_out_embeddings.weight
    new_out_embeddings.weight = offloaded_W
    new_out_embeddings.in_features  = offloaded_W.shape[1]
    new_out_embeddings.out_features = offloaded_W.shape[0]

    new_out_embeddings._offloaded_file_location = offloaded_W._offloaded_file_location
    model.set_out_embeddings(new_out_embeddings)
    return
pass
