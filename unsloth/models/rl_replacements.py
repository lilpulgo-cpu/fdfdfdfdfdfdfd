...
def sft_trainer_compute_loss(function_name, function):
    ...
    def compute_loss(self, model, inputs, return_outs = False, num_items_in_batch = None):
        outs = super().compute_loss(
            model,
            inputs,
...
def grpo_trainer__prepare_inputs(function_name, function):
    ...
    function = function.replace(
        ...
        "if not torch.is_autocast_enabled('') else nullcontext():",
    )
...
def grpo_trainer__get_per_token_logps(function_name, function):
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, logits_to_keep):
        ...
        with torch.amp.autocast(device_type = '', dtype = self._autocast_dtype):
...
def grpo_trainer_compute_loss(function_name, function):
    ...
    def compute_loss(self, model, inputs, return_outs = False, num_items_in_batch = None):
        if return_outs:
            raise ValueError("The GRPOTrainer does not support returning outs")
