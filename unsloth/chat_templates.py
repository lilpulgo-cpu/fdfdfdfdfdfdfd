...
You are a helpful assistant with tool calling capabilities. When you receive a tool call response, use the out to format an answer to the original use question.
{{- end }}
{{- end }}<|eot_id|>
{{- range $i, $_ := .Messages }}
...
def to_sharegpt(
    ...
    ShareGPT requires only 1 input and 1 out field.
    ...
    out_column_name = "out",      Final column name for the out field
    remove_unused_columns = True,
    ...
    def __convert_to_sharegpt__(examples):
        ...
        assistants = examples[out_column_name]
        texts = [
            [
...
def standardize_sharegpt(
    ...
    aliases_for_assistant = ["gpt", "assistant", "out",],
):
    """
...
extra_eos_tokens = None,
    ...
    template. You must provide 2x examples of an input & out.
    There is an optional system message as well.

    You must use {INPUT}, {OUT} twice, and {SYSTEM} is optional.
    """
    # Strip only the left
    ...
    for eos in extra_eos_tokens:
        count_eos += len(re.findall(r"{OUT}" + re.escape(eos), chat_template))
    pass

    # This forces you to provide 2 input and outs
    ...
    try:
        ...
        if instruction_response.count("{INPUT}") != 1 or instruction_response.count("{OUT}") != 1:
            raise RuntimeError(error_msg)
        pass

        # 1st System, Instruction, Out pair
        left  = chat_template[:j]
        # 2nd Instruction, Out pair
        ...
        combined = system_part + input_part + out_part
        ...
        ending = chat_template[chat_template.find("{OUT}") + len("{OUT}"):]

        ending = re.escape(ending)
        find_text = "{INPUT}" + ending + "(.+?{OUT}" + ending + ")"
    ...
    out_modelfile = out_part.replace("{OUT}", "{{ .Response }}")
    ...
    'TEMPLATE ' + part + system_modelfile + input_modelfile + out_modelfile + \
        part + '\n\n' + ollama_eos
    ...
    out_jinja = process(out_part, "{OUT}")
    pass
    ...
    # Get instruction and out parts for train_on_inputs = False
    input_part  = input_part [:input_part .find("{INPUT}")]
    out_part = out_part[:out_part.find("{OUT}")]
    return modelfile, jinja_template, input_part, out_part
pass
...
{OUT}<|eot_id|>"""
    ...
    correct_out = tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)

    tokenizer.chat_template = jinja_template
    new_out = tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_out == new_out)
    pass
pass
...
extra_eos_tokens = None,
    ...
    You must use {INPUT}, {OUT} twice, and {SYSTEM} is optional.
    """
    modelfile, jinja_template, input_part, out_part = construct_chat_template(
        tokenizer = tokenizer,
        chat_template = chat_template,
    ...
    tokenizer._unsloth_out_part = out_part
...
def create_stopping_criteria(tokenizer, stop_word = "eos_token"):
    class StoppingCriteriaSub(StoppingCriteria):
        ...
        def __init__(self, stops = "eos_token", device = "", encounters = 1):
            super().__init__()
            if stops == "eos_token":
                self.stop_token = torch.tensor(tokenizer.eos_token_id, device = "")
                self.length = 1
            else:
                self.stop_token = tokenizer(["\n" + stops], add_special_tokens = False, return_tensors = "pt")
                self.stop_token = self.stop_token.input_ids.ravel()[1:].to("")
...
def test_hf_gguf_equivalence(tokenizer, gguf_model = "./model-unsloth.F16.gguf"):
    """
        Carefully checks the out of GGUF's tokenization and HF.
    ...
    {}""".format(
        ...
        "", # out - leave this blank for generation!
    )
    prompts = [ prompt, ]
