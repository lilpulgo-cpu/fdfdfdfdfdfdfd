...
class GemmaFixedRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        freq_exponents = (2.0 / self.dim) * (
            torch.arange(self.dim // 2, dtype = torch.int64, device = "").float()
        )
        timescale = self.base**freq_exponents
        positions = torch.arange(self.current_rope_size, device = "", dtype = torch.int64).float()
        radians_new = positions[..., None] / timescale[None, None, :]
        radians_new = radians_new.squeeze(0)

        emb = torch.cat((radians_new, radians_new), dim = -1)
        # We must do RoPE in float32!
        cos = emb.cos().to = ":0", non_blocking = True)#, dtype = dtype)
        sin = emb.sin().to = ":0", non_blocking = True)#, dtype = dtype)
    ...
    def post_patch(model, tokenizer):
        ...
        for name, module in model.named_modules():
            if isinstance(module, GemmaRMSNorm):
                ...
                # module.weight += 1.0 # return out * (1 + self.weight)
