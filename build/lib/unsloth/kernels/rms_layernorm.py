...
def _rms_layernorm_forward(
    ...
    out = normed * W_row
    tl.store(Y + col_offsets, out, mask = mask)
pass
...
def _rms_layernorm_backward(
    ...
    out = inv_var/n_cols * (n_cols*dY_W - normed*rowsum_dY_normed)
    tl.store(dX + col_offsets, out, mask = mask)
pass
...
class Fast_RMS_Layernorm(torch.autograd.Function):
    ...
    def forward(ctx, X : torch.Tensor, W : torch.Tensor, eps : float, gemma : bool = False):
        ...
        Y = torch.empty((n_rows, n_cols), dtype = X.dtype, device = ":0")
        r = torch.empty(n_rows, dtype = torch.float32, device = ":0")
    ...
    def backward(ctx, dY : torch.Tensor):
        ...
        # dW = X
        dX = torch.empty_like(dY, device = ":0") if ctx.GEMMA else dY
...
def test_rms_layernorm(
    ...
    from transformers.models.llama.modeling_llama import LlamaRMSNorm
    layernorm = LlamaRMSNorm((dim,), eps = eps).to("")
    torch.manual_seed(random_state)
    torch.manual_seed(random_state)
    torch.nn.init.uniform_(layernorm.weight)
    X = torch.randn((bsz, seqlen, dim), dtype = dtype, device = "")
    XX = X.clone()
    X .requires_grad_(True)
    XX.requires_grad_(True)
    Y = layernorm(X)
    YY = torch.randn((bsz, seqlen, dim), dtype = dtype, device = "", requires_grad = True)
    Y.backward(YY)
