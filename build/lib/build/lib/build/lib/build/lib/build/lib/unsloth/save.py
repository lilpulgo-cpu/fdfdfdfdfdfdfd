...
def unsloth_save_model(
    ...
    n_s = psutil._count(logical = False)
    if n_s is None: n_s = psutil._count()
    if n_s is None: n_s = 1

    if safe_serialization is None:
        safe_serialization = True
        save_pretrained_settings["safe_serialization"] = safe_serialization

    elif safe_serialization and (n_s <= 2):
        logger.warning_once(
            f"Unsloth: You have {n_s} s. Using `safe_serialization` is 10x slower.\n"\
    ...
    max_vram = int(torch..get_device_properties(0).total_memory * maximum_memory_usage)
    ...
    for j, layer in enumerate(ProgressBar(internal_model.model.layers)):
        for item in LLAMA_WEIGHTS:
            ...
            if (torch..memory_allocated() + W.nbytes) < max_vram:
                # Save to  memory
                state_dict[name] = W
            ...
            #     state_dict[name] = W.to("", non_blocking = True, copy = True)
            #     max_ram = max(max_ram - W.nbytes, 0)
                ...
                state_dict[name] = torch.load(filename, map_location = "", mmap = True, weights_only = False)
        pass
...
def install_llama_cpp_make_non_blocking():
    ...
    # Weirdly  conversion for GGUF breaks??
    # env = { **os.environ, "LLAMA_": "1", }
    # Force make clean
    check = os.system("make clean -C llama.cpp")
    IS_CMAKE = False
    if check == 0:
        # Uses old MAKE
        n_jobs = max(int(psutil._count()*1.5), 1)
        full_command = ["make", "all", "-j"+str(n_jobs), "-C", "llama.cpp"]
        IS_CMAKE = False
    else:
        # Uses new CMAKE
        n_jobs = max(int(psutil._count()), 1) # Use less s since 1.5x faster
        check = os.system("cmake llama.cpp -B llama.cpp/build -DBUILD_SHARED_LIBS=OFF -DGGML_=OFF -DLLAMA_CURL=ON")
        if check != 0:
            raise RuntimeError(f"*** Unsloth: Failed compiling llama.cpp using os.system(...) with error {check}. Please report this ASAP!")
        pass
        # f"cmake --build llama.cpp/build --config Release -j{psutil._count()*2} --clean-first --target {' '.join(LLAMA_CPP_TARGETS)}",
        full_command = [
...
def install_llama_cpp_old(version = -10):
    ...
    # FALLBACK mechanism
    releases = subprocess.check_out(["git", "ls-remote", "--tags", "https://github.com/ggerganov/llama.cpp.git"])
    ...
    commands = [
        ...
        f"make all -j{psutil._count()*2} -C llama.cpp",
    ]
    if try_execute(commands) == "CMAKE":
        # Instead use CMAKE
        commands = [
            "cmake llama.cpp -B llama.cpp/build -DBUILD_SHARED_LIBS=OFF -DGGML_=OFF -DLLAMA_CURL=ON",
            f"cmake --build llama.cpp/build --config Release -j{psutil._count()*2} --clean-first --target {' '.join(LLAMA_CPP_TARGETS)}",
...
def install_llama_cpp_blocking(use_ = False):
    # https://github.com/ggerganov/llama.cpp/issues/7062
    # Weirdly  conversion for GGUF breaks??
    # use_ = "LLAMA_=1" if use_ else ""
    ...
    commands = [
        ...
        # https://github.com/ggerganov/llama.cpp/issues/7062
        # Weirdly  conversion for GGUF breaks??
        # f"{use_} make all -j{psutil._count()*2} -C llama.cpp",
        f"make all -j{psutil._count()*2} -C llama.cpp",
    ]
    if try_execute(commands) == "CMAKE":
        # Instead use CMAKE
...
def save_to_gguf(
    ...
    print(f"Unsloth: [1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
          f"The out location will be {final_location}\n"\
    ...
    if use_fast_convert:
        command = f"python llama.cpp/convert.py {model_directory} "\
            ...
            f"--outtype {first_conversion} --concurrency {n_s} --pad-vocab"
    else:
    ...
    if not os.path.isfile(final_location):
        ...
        pass
    pass
    print(f"Unsloth: Conversion completed! Out location: {final_location}")
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            ...
            command = f"./{quantize_location} {full_precision_location} "\
                f"{final_location} {quant_method} {n_s}"
...
def unsloth_save_pretrained_gguf(
    ...
    if not os.path.exists("llama.cpp"):
        ...
        if IS_KAGGLE_ENVIRONMENT:
            # Kaggle is weird - no blocking installs, and no ?
            python_install = install_python_non_blocking(["gguf", "protobuf"])
            python_install.wait()
            install_llama_cpp_blocking(use_ = False)
...
def unsloth_convert_lora_to_ggml_and_push_to_hub(
    ...
    model_type = self.config.model_type
    out_file = os.path.join(lora_directory_push, "ggml-adapter-model.bin")

    print(f"Unsloth: Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    print(f"The out file will be {out_file}")

    command = f"python3 llama.cpp/convert-lora-to-ggml.py {lora_directory_push} {out_file} llama"
    ...
    print(f"Unsloth: Conversion completed! Out file: {out_file}")

    print("Unsloth: Uploading GGML file to Hugging Face Hub...")
    username = upload_to_huggingface(
        self, repo_id, token,
        "GGML converted LoRA", "ggml", out_file, None, private,
    )
...
def unsloth_convert_lora_to_ggml_and_save_locally(
    ...
    model_type = self.config.model_type
    out_file = os.path.join(save_directory, "ggml-adapter-model.bin")

    print(f"Unsloth: Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The out file will be {out_file}")

    command = f"python3 llama.cpp/convert-lora-to-ggml.py {save_directory} {out_file} llama"
