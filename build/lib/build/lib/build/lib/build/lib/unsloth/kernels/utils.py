...
HAS__STREAM = Version(bnb.__version__) > Version("0.43.3")
global _STREAM
_STREAM = None
...
if HAS__STREAM:
    ...
    def fast_dequantize(W, quant_state = None, out = None, use_global_buffer = False):
        ...
        pass
        global _STREAM
        if _STREAM is None: _STREAM = torch.current_stream(":0")
        ...
        if use_global_buffer:
            ...
            if WEIGHT_BUFFER is None:
                WEIGHT_BUFFER = torch.empty(size, dtype = dtype,  = ":0", requires_grad = False)
                ABSMAX_BUFFER = torch.empty(n_elements_absmax, dtype = torch.float32,  = ":0", requires_grad = False)
            ...
            if out is None:
                out = torch.empty(shape, dtype = dtype,  = ":0", requires_grad = False)
            else:
                assert(out.shape == shape)
                assert(out.dtype == dtype)
            out_absmax = torch.empty(n_elements_absmax, dtype = torch.float32,  = ":0", requires_grad = False)
        pass
        ...
        cdequantize_blockwise_fp32(
            ...
            ctypes_c_int(blocksize2), ctypes_c_int(n_elements_absmax), _STREAM,
        )
        out_absmax += offset
        ...
        fx(get_ptr(None), get_ptr(W), ptr_out_absmax, get_ptr(out),
           ctypes_c_int(blocksize), ctypes_c_int(out.numel()), _STREAM,)
    ...
    def fast_dequantize(W, quant_state = None, out = None, use_global_buffer = False):
        ...
        if use_global_buffer:
            ...
            if WEIGHT_BUFFER is None:
                WEIGHT_BUFFER = torch.empty(size, dtype = dtype,  = ":0", requires_grad = False)
                ABSMAX_BUFFER = torch.empty(n_elements_absmax, dtype = dtype,  = ":0", requires_grad = False)
...
if HAS__STREAM:
    def fast_gemv(X, W, quant_state, out = None):
        ...
        if out is None:
            out = torch.empty((1, 1, bout,), dtype = dtype,  = ":0")
        # else:
        ...
        df = torch.empty(absmax.shape, dtype = torch.float32,  = ":0")
        cdequantize_blockwise_fp32(
